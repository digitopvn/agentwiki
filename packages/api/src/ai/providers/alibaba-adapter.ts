/** Alibaba DashScope API adapter (OpenAI-compatible mode) */
...
export class AlibabaAdapter implements AIProvider {
  readonly id = 'alibaba'
  ...
  async generateText(apiKey: string, req: AIRequest): Promise<AIResponse> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Alibaba DashScope API error ${res.status}: ${err}`)
    }
